...
class OracleSandboxAuthenticatedReadServiceTest {
    ...
    void disabledProfileDoesNotCallOracle() {
        FhirAuthenticatedReadResult result = service().searchPatients(OracleHealthIntegrationProfileTest.completePublicPkce());
    ...
    void missingTokenIsAuthenticationRequiredWithoutHttp() {
        ...
        FhirAuthenticatedReadResult result =
                service().searchPatients(OracleHealthIntegrationProfileTest.completePublicPkceEnabled());
    ...
    @Test
    void missingPatientCapabilityIsUnsupportedWithoutSearch() {
        usableToken();
        ...
        assertThat(result.outcome()).isEqualTo(FhirAuthenticatedReadOutcome.CAPABILITY_UNSUPPORTED);
        verify(routingService, never()).searchPatients(any(), any(), any());
    }

    @Test
    void missingSearchTypeIsUnsupportedWithoutSearch() {
        usableToken();
    ...
    @Test
    void validTokenAndCapabilityExecuteGenericSearch() {
        usableToken();
        ...
        when(routingService.searchPatients(any(), any(), any())).thenReturn(bundle);
        ...
        verify(routingService)
                .searchPatients(any(), any(), eq(OracleSandboxAuthenticatedReadService.SAFE_PATIENT_SEARCH_NAME));
    }
    ...
    void http401IsAuthenticationRejected() {
        ...
        when(capabilityDiscovery.discover(any()))
                .thenReturn(new lab.healthcare.fhir.capability.FhirCapabilityDiscoveryService()
                        .interpret("oracle-health-sandbox", FhirCapabilityDiscoveryServiceTest.sampleStatement()));
        when(routingService.searchPatients(any(), any(), any()))
    ...
    private OracleSandboxAuthenticatedReadService service() {
        return new OracleSandboxAuthenticatedReadService(
