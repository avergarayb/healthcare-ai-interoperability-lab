...
/**
 * Laboratory surfaces for clinical data-access enforcement decisions.
 * Deny-by-default. Not real access, not FHIR, and not a model runtime.
 */
@RestController
public class AiConsumerClinicalDataEnforcementController {

    private static final Logger log = LoggerFactory.getLogger(AiConsumerClinicalDataEnforcementController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerClinicalDataEnforcementController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(path = "/lab/ai-consumer-clinical-data-enforcement", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerClinicalDataEnforcement(evaluated.result()));
    }

    @GetMapping(path = "/api/ai-consumer-clinical-data-enforcement/v1", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerClinicalDataEnforcementResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        AiConsumerClinicalDataEnforcementResult result = AiConsumerClinicalDataEnforcementBoundary.evaluate(access);
        log.info(
                "AI consumer clinical data enforcement status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerClinicalDataEnforcementResult result, int httpStatus) {
    }
}
