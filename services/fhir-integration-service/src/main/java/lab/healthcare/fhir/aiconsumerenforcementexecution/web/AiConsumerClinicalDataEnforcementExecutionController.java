 ...
 */
@RestController
public class AiConsumerClinicalDataEnforcementExecutionController {

    private static final Logger log =
            LoggerFactory.getLogger(AiConsumerClinicalDataEnforcementExecutionController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerClinicalDataEnforcementExecutionController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(path = "/lab/ai-consumer-clinical-data-enforcement-execution", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerClinicalDataEnforcementExecution(evaluated.result()));
    }

    @GetMapping(
            path = "/api/ai-consumer-clinical-data-enforcement-execution/v1",
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerClinicalDataEnforcementExecutionResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        log.info(
                "AI consumer clinical data enforcement execution status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerClinicalDataEnforcementExecutionResult result, int httpStatus) {
    }
}
