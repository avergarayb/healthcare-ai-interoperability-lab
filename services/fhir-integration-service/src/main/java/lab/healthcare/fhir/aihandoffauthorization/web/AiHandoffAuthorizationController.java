...
/**
 * Laboratory surfaces for the handoff authorization boundary. Deny-by-default.
 * Not real authorization, not handoff, and not a model runtime.
 */
@RestController
public class AiHandoffAuthorizationController {

    private static final Logger log = LoggerFactory.getLogger(AiHandoffAuthorizationController.class);

    private final ModelBoundaryContractProvider provider;

    public AiHandoffAuthorizationController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(path = "/lab/ai-handoff-authorization", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiHandoffAuthorization(evaluated.result()));
    }

    @GetMapping(path = "/api/ai-handoff-authorization/v1", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiHandoffAuthorizationResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        AiHandoffAuthorizationResult result = AiHandoffAuthorizationBoundary.evaluate(readiness);
        log.info(
                "AI handoff authorization status={} reason={} readiness={} policy={} version={} consumerType={} operation={} scope={} identityPresent={} authenticated={} authorized={} tenant={} externalAuth={} handoffAuthorized={} dispatchPerformed={} modelCalled={} modelCallAuthorized={} dispatch={}",
                ...
                result.consumerIdentityPresent(),
                result.consumerAuthenticated(),
                result.consumerAuthorized(),
                result.tenantContextPresent(),
                result.externalAuthorizationAvailable(),
    ...
    private record Evaluated(AiHandoffAuthorizationResult result, int httpStatus) {
    }
}
