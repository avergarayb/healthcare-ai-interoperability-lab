...
/**
 * Laboratory surfaces for clinical data-access enforcement
 * verification-decision. Deny-by-default. Not real access, not FHIR,
 * and not a model runtime.
 */
@RestController
public class AiConsumerVerificationDecisionController {

    private static final Logger log = LoggerFactory.getLogger(AiConsumerVerificationDecisionController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerVerificationDecisionController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(
            path = "/lab/ai-consumer-clinical-data-enforcement-verification-decision",
            produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerVerificationDecision(evaluated.result()));
    }

    @GetMapping(
            path = "/api/ai-consumer-clinical-data-enforcement-verification-decision/v1",
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerVerificationDecisionResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        log.info(
                "AI consumer clinical data enforcement verification decision status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerVerificationDecisionResult result, int httpStatus) {
    }
}
