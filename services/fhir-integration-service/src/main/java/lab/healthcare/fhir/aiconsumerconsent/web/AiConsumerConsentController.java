...
/**
 * Laboratory surfaces for consumer consent and purpose. Deny-by-default.
 * Not real consent, not clinical access, and not a model runtime.
 */
@RestController
public class AiConsumerConsentController {

    private static final Logger log = LoggerFactory.getLogger(AiConsumerConsentController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerConsentController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(path = "/lab/ai-consumer-consent", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerConsent(evaluated.result()));
    }

    @GetMapping(path = "/api/ai-consumer-consent/v1", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerConsentResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        AiConsumerConsentResult result = AiConsumerConsentBoundary.evaluate(authorization);
        log.info(
                "AI consumer consent status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerConsentResult result, int httpStatus) {
    }
}
