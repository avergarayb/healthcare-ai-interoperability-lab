...
/**
 * Laboratory surfaces for clinical data-access enforcement-execution
 * verification. Deny-by-default. Not real access, not FHIR, and not a
 * model runtime.
 */
@RestController
public class AiConsumerClinicalDataEnforcementExecutionVerificationController {

    private static final Logger log =
            LoggerFactory.getLogger(AiConsumerClinicalDataEnforcementExecutionVerificationController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerClinicalDataEnforcementExecutionVerificationController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(
            path = "/lab/ai-consumer-clinical-data-enforcement-execution-verification",
            produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerClinicalDataEnforcementExecutionVerification(evaluated.result()));
    }

    @GetMapping(
            path = "/api/ai-consumer-clinical-data-enforcement-execution-verification/v1",
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerClinicalDataEnforcementExecutionVerificationResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        log.info(
                "AI consumer clinical data enforcement execution verification status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(
            AiConsumerClinicalDataEnforcementExecutionVerificationResult result, int httpStatus) {
    }
}
