...
/**
 * Laboratory surfaces for enforcement verification-approval. Deny-by-default.
 * Not real access, not FHIR, and not a model runtime.
 */
@RestController
public class AiConsumerEnforcementVerificationApprovalController {

    private static final Logger log =
            LoggerFactory.getLogger(AiConsumerEnforcementVerificationApprovalController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerEnforcementVerificationApprovalController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(
            path = "/lab/ai-consumer-enforcement-verification-approval",
            produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerEnforcementVerificationApproval(evaluated.result()));
    }

    @GetMapping(
            path = "/api/ai-consumer-enforcement-verification-approval/v1",
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerEnforcementVerificationApprovalResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        AiConsumerClinicalDataAccessResult access = AiConsumerClinicalDataAccessBoundary.evaluate(scope);
        AiConsumerClinicalDataEnforcementResult enforcement =
                AiConsumerClinicalDataEnforcementBoundary.evaluate(access);
        AiConsumerClinicalDataEnforcementExecutionResult execution =
                AiConsumerClinicalDataEnforcementExecutionBoundary.evaluate(enforcement);
        AiConsumerClinicalDataEnforcementExecutionVerificationResult verification =
                AiConsumerClinicalDataEnforcementExecutionVerificationBoundary.evaluate(execution);
        AiConsumerVerificationDecisionResult decision =
                AiConsumerVerificationDecisionBoundary.evaluate(verification);
        AiConsumerEnforcementVerificationApprovalResult result =
                AiConsumerEnforcementVerificationApprovalBoundary.evaluate(decision);
        log.info(
                "AI consumer enforcement verification approval status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerEnforcementVerificationApprovalResult result, int httpStatus) {
    }
}
