...
/**
 * Laboratory surfaces for consumer authentication and authorization.
 * Deny-by-default. Not real authentication, not handoff, and not a model runtime.
 */
@RestController
public class AiConsumerAuthorizationController {

    private static final Logger log = LoggerFactory.getLogger(AiConsumerAuthorizationController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerAuthorizationController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(path = "/lab/ai-consumer-authorization", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerAuthorization(evaluated.result()));
    }

    @GetMapping(path = "/api/ai-consumer-authorization/v1", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerAuthorizationResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        AiConsumerAuthorizationResult result = AiConsumerAuthorizationBoundary.evaluate(handoff);
        log.info(
                "AI consumer authorization status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerAuthorizationResult result, int httpStatus) {
    }
}
