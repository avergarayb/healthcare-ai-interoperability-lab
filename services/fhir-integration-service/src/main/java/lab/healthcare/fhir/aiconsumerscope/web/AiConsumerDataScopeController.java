...
/**
 * Laboratory surfaces for clinical data-scope and minimization.
 * Deny-by-default. Not real access, not FHIR, and not a model runtime.
 */
@RestController
public class AiConsumerDataScopeController {

    private static final Logger log = LoggerFactory.getLogger(AiConsumerDataScopeController.class);

    private final ModelBoundaryContractProvider provider;

    public AiConsumerDataScopeController(ModelBoundaryContractProvider provider) {
        if (provider == null) {
    ...
    @GetMapping(path = "/lab/ai-consumer-data-scope", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> labPage() {
        Evaluated evaluated = evaluate();
        return ResponseEntity.status(evaluated.httpStatus())
                .body(SmartLabPages.aiConsumerDataScope(evaluated.result()));
    }

    @GetMapping(path = "/api/ai-consumer-data-scope/v1", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AiConsumerDataScopeResult> payload() {
        Evaluated evaluated = evaluate();
    ...
    private Evaluated evaluate() {
        ...
        AiConsumerDataScopeResult result = AiConsumerDataScopeBoundary.evaluate(consent);
        log.info(
                "AI consumer data scope status={} reason={} operation={}",
                result.status(),
                result.reason(),
                result.operation());
    ...
    private record Evaluated(AiConsumerDataScopeResult result, int httpStatus) {
    }
}
