...
`fhir-integration-service` is a Java FHIR **client**. See [fhir-client.md](fhir-client.md) for `/metadata`, [fhir-search.md](fhir-search.md) for Patient read/search, [fhir-resources-and-references.md](fhir-resources-and-references.md) for Observation, Condition, and `Reference`, [fhir-include-revinclude.md](fhir-include-revinclude.md) for `_include` and `_revinclude`, [fhir-crud-write-operations.md](fhir-crud-write-operations.md) for create, update, and delete, and [fhir-advanced-search.md](fhir-advanced-search.md) for multiple parameters, modifiers, date prefixes, `_sort`, and `_count`.
