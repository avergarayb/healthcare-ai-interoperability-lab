...
| `vendor.epic` | Epic sandbox profile, SMART start/callback orchestration, readiness, public metadata capability discovery, controlled Patient context and read | Patient search, Hyperspace, `private_key_jwt` |
...
vendor.epic ──► routing     (generic Patient READ with issued AccessTokenProvider)
...
| `vendor.epic` | Epic profile / validator unit tests, `EpicIntegrationProfileIT`, sandbox SMART, capability-discovery, and controlled Patient-read ITs |
