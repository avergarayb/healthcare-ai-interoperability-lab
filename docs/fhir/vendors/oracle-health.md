...
## Authenticated Observation search by Patient (Task 038)

Condition access does not imply Observation access. The same Patient context and token are reused; the capability and scope are checked independently.
...
No `GET /Observation` without `patient`. There is no `OracleObservationClient`. Additional vendor search parameters are not added unless Oracle rejects this query.

HTTP 403 usually means the token lacks `user/Observation.read` in Code Console and `.env`. The 60-second client socket timeout from Task 037 is reused as a generic transport policy.

### Diagnosis

| Outcome | Meaning |
|---|---|
| `OBSERVATION_SEARCH_SUCCEEDED` | Oracle returned a FHIR Bundle — JSON is not rendered |
| `PATIENT_CONTEXT_NOT_CONFIGURED` | No sandbox Patient ID — no Observation HTTP |
| `AUTHENTICATION_REQUIRED` | No usable token (or sandbox disabled) |
| `AUTHENTICATION_REJECTED` | HTTP 401 |
| `AUTHORIZATION_DENIED` | HTTP 403 |
| `CAPABILITY_UNSUPPORTED` | Runtime model lacks Observation `search-type` |
| `DEPENDENCY_FAILURE` | Timeout, connection, 5xx, rate limit (existing taxonomy) |

Lab page: `GET /oracle/sandbox/fhir/observation-search` after SMART login and a configured Patient ID.

## Authenticated DiagnosticReport search by Patient (Task 039)

Observation access does not imply DiagnosticReport access. The same Patient context and token are reused; the capability and scope are checked independently.
...
No `GET /DiagnosticReport` without `patient`. There is no `OracleDiagnosticReportClient`. Additional vendor search parameters are not added unless Oracle rejects this query.

HTTP 403 usually means the token lacks `user/DiagnosticReport.read` in Code Console and `.env`. The 60-second client socket timeout from Task 037 is reused as a generic transport policy.

### Diagnosis

| Outcome | Meaning |
|---|---|
| `DIAGNOSTIC_REPORT_SEARCH_SUCCEEDED` | Oracle returned a FHIR Bundle — JSON is not rendered |
| `PATIENT_CONTEXT_NOT_CONFIGURED` | No sandbox Patient ID — no DiagnosticReport HTTP |
| `AUTHENTICATION_REQUIRED` | No usable token (or sandbox disabled) |
| `AUTHENTICATION_REJECTED` | HTTP 401 |
| `AUTHORIZATION_DENIED` | HTTP 403 |
| `CAPABILITY_UNSUPPORTED` | Runtime model lacks DiagnosticReport `search-type` |
| `DEPENDENCY_FAILURE` | Timeout, connection, 5xx, rate limit (existing taxonomy) |
...
## Controlled clinical projection (Task 042)

Task 041 can receive more entries than requested (`_count=5` is a request, not a ceiling). Task 042 keeps the same sequential operations and applies an application retention ceiling of `N = 5` plus an explicit allowlist:
...
The EHR may still return 1489 Conditions. The application retains at most five allowlisted records. First N is operational, not clinical ranking. Empty Bundles remain `SUCCESS` with `receivedCount=0`, `retainedCount=0`, `truncated=false`. There is no `OracleProjectionClient` and no AI-ready context.

The laboratory page must not show projected field values.

Lab page: `GET /oracle/sandbox/fhir/clinical-projection` after SMART login and a configured Patient ID. HTTP 200 for complete **and** partial. The page does not show Patient ID, projected values, or clinical JSON.

## Vendor-neutral model boundary (Task 043)

Task 042 retains at most five allowlisted records. Task 043 copies that projection onto a single generic contract (`lab.healthcare.fhir.modelboundary`, `contractVersion=v1`) without a second FHIR fetch:
...
generic mapper
        ↓
vendor-neutral model boundary
        ↓
STOP
```

The contract shape does not change for `oracle-health-sandbox`, `epic-sandbox`, or another FHIR destination. There is no `OracleModelBoundaryClient`, no Epic connection, and no LLM call. Empty and partial upstream states are preserved. Task 055 compares Oracle and Epic against the same v1 invariants; Oracle may include MedicationRequest while Epic omits it. See [model-boundary-contract-v1.md](../model-boundary-contract-v1.md). Task 056 diagnoses those results with the same vendor-neutral pipeline statuses; a present Oracle MedicationRequest is `SUCCESS` or another collection status, never rewritten as Epic absence. Task 057 evaluates the same Oracle-backed contract at `GET /lab/deterministic-agent` without an `OracleAgent` or a model call. Task 058 exposes that verdict at `GET /lab/ai-boundary` with `modelCallAuthorized=false`. Task 059 consumes the same Oracle-backed boundary at `GET /lab/first-ai-component` without an `OracleAi` or a model call (`processingStatus=NOT_EXECUTED`). Task 060 evaluates that result at `GET /lab/ai-execution-gate` as `ELIGIBLE_BUT_NOT_AUTHORIZED` without authorizing a model. Task 061 prepares the same Oracle-backed verdict as AI Consumer Contract v1 at `GET /lab/ai-consumer-contract` with `dispatchStatus=NOT_DISPATCHED`. Task 062 evaluates a synthetic laboratory consumer at `GET /lab/ai-consumer-policy` as `ALLOWED_FOR_FUTURE_CONSUMPTION` without authorizing a model. Task 063 evaluates readiness over that policy at `GET /lab/ai-consumer-readiness` as `READY_FOR_FUTURE_HANDOFF` without authorizing handoff or dispatch. Task 064 evaluates a deny-by-default authorization boundary at `GET /lab/ai-handoff-authorization` as `HANDOFF_NOT_AUTHORIZED`. Task 065 evaluates consumer authentication and authorization at `GET /lab/ai-consumer-authorization` as `AUTHORIZATION_NOT_IMPLEMENTED`. Task 066 evaluates consent and purpose at `GET /lab/ai-consumer-consent` as `CONSENT_NOT_IMPLEMENTED`. Task 067 evaluates clinical data-scope and minimization at `GET /lab/ai-consumer-data-scope` as `NOT_READY_FOR_CLINICAL_DATA_ACCESS`. Task 068 evaluates a clinical data-access request at `GET /lab/ai-consumer-clinical-data-access` as `NOT_GRANTED_FOR_CLINICAL_DATA_ACCESS`. Task 069 evaluates a clinical data-access enforcement decision at `GET /lab/ai-consumer-clinical-data-enforcement` as `NOT_ENFORCED_FOR_CLINICAL_DATA_ACCESS`. Task 070 evaluates enforcement execution at `GET /lab/ai-consumer-clinical-data-enforcement-execution` as `NOT_EXECUTED_FOR_CLINICAL_DATA_ACCESS`. Task 071 evaluates enforcement-execution verification at `GET /lab/ai-consumer-clinical-data-enforcement-execution-verification` as `NOT_VERIFIED_FOR_CLINICAL_DATA_ACCESS`. Task 072 evaluates a verification decision at `GET /lab/ai-consumer-clinical-data-enforcement-verification-decision` as `VERIFICATION_DECISION_NOT_AVAILABLE`. Task 073 evaluates verification approval at `GET /lab/ai-consumer-enforcement-verification-approval` as `VERIFICATION_APPROVAL_NOT_AVAILABLE`.
...
## Contract-consuming agent stub (Task 045)

A vendor-neutral stub consumes the v1 contract through `ModelBoundaryContractProvider` and stops. It does not call a model and does not republish `records`.

```text
GET /lab/agent-stub          HTML observation (counts only)
GET /api/agent-stub/v1       JSON AgentStubObservation
```

`modelCalled` is always false. Oracle is only the current provider behind the contract. There is no Python `ai-service` and no HTTP self-call to `/api/model-boundary/v1`.
