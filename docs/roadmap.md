...
Phase 2 starts with **named FHIR server profiles** and external configuration (`local-hapi` today). OAuth 2.0 Client Credentials is available on the optional `secured-lab` profile. SMART on FHIR Authorization Code + PKCE is available on the optional `smart-lab` profile. An Epic sandbox destination is a disabled vendor profile only.
