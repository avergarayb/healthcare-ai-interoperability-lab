...
The repository is in Phase 2–3. `fhir-integration-service` remains a FHIR R4 **client**. Named server profiles (`fhir.active-server`) select connectivity. OAuth 2.0 Client Credentials is optional (`secured-lab`). SMART Authorization Code + PKCE is optional (`smart-lab`). Java packages are split by responsibility (`client`, `server`, `auth`, `auth.oauth2`, `smart`, `mapping`, `exception`); see [`docs/fhir/fhir-architecture.md`](fhir/fhir-architecture.md). External JSON can be mapped to HAPI R4 `Patient` / `Observation` without changing `FhirService`; see [`docs/fhir/fhir-mapping.md`](fhir/fhir-mapping.md). Neither talks to a commercial EHR. Local HAPI does not ship LOINC/SNOMED CodeSystems.
