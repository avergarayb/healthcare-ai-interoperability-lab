...
The repository is in Phase 1. `fhir-integration-service` can search, write, chain, `_has`, call `$validate-code`, call `$validate`, send Bundle transaction/batch interactions, and page `searchset` Bundles against local HAPI FHIR. Local HAPI does not ship LOINC/SNOMED CodeSystems.
